<template>
  <div
    ...
    @click="navigateTo(`/mobile/${mobile.title}`)"
  >
    <div class="flex flex-col h-full bg-zinc-100 rounded-2xl shadow-md">
      <NuxtImg
        :src="`https://kgptltaqnkidtcqkathb.supabase.co/storage/v1/object/public/a7-images/mobile/${mobile.figure}`"
        alt=""
      ...
      <div class="p-4 flex flex-col">
        <div>
          <p class="mt-auto">類別: {{ mobile.category }}</p>
          <p class="bg-blue-200 hover:bolder-red px-3 py-2 rounded text-lg">
            <a :href="mobile.link"> {{ mobile.title }} </a>
          </p>
        </div>
      </div>
    </div>
